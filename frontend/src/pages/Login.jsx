    ...
    const handleForm = (e) => {
        ...
        axios.post(`${API}/api/token/`, { username: email, password })
            .then((res => {
                ...
                // Ahora pedimos el rol
                axios.get(`${API}/api/verificar_rol/`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                    .then(res => {
                        const rol = res.data.rol
                        setLogueado(true)  // actualiza estado global

                        // Redireccionamos según el rol
                        if (rol === 'A') window.location.href = '/admin_panel'
                        else if (rol === 'B') window.location.href = '/barber_panel'
                        else window.location.href = '/'
                    })
                    .catch(() => {
                        setErrores('Error al obtener el rol')
                        localStorage.clear()
                    })
            }))
            .catch((error => {
